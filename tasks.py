...
async def start_tasks_when_ready():
    ...
    try:
        # A chamada a wait_until_ready() já garante que o bot está conectado e o setup_hook terminou.
        await bot.wait_until_ready()
        ...
        logger.error(f"Erro ao iniciar tasks: {e}", exc_info=True)
        # Não levante a exceção aqui, apenas registre o erro.
        # raise
...
def setup_tasks():
    ...
    try:
        ...
        # Se não houver um loop de eventos atual, o bot criará um.
        pass
    
    # A inicialização das tasks será tratada pelo setup_hook do bot.
    # Esta função agora apenas agenda a inicialização para quando o loop estiver rodando.
    # O bot.loop.create_task precisa ser chamado quando o loop já está em execução.
    # A chamada será feita pelo setup_hook em main.py.
    # Apenas garantimos que o start_tasks_when_ready seja chamado.
    
    # CORREÇÃO: A lógica de agendamento é complexa com a thread do Flask.
    # A forma mais simples é chamar 'start_tasks_when_ready' diretamente.
